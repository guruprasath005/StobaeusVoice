...
@router.get("")
def list_prescriptions(patient_id: Optional[str] = None, limit: int = 40, db: Session = Depends(get_db), _: User = Depends(get_current_user)):
...
@router.post("")
def create_prescription(req: CreateRxRequest, db: Session = Depends(get_db), _: User = Depends(get_current_user)):
...
@router.get("/{rx_id}")
def get_prescription(rx_id: str, db: Session = Depends(get_db), _: User = Depends(get_current_user)):
...
@router.patch("/{rx_id}")
def update_prescription(rx_id: str, req: UpdateRxRequest, db: Session = Depends(get_db), _: User = Depends(get_current_user)):
    rx = db.query(Prescription).filter(Prescription.rx_id == rx_id).first()
    if not rx:
        raise HTTPException(status_code=404, detail="Prescription not found")
...
def send_whatsapp(rx_id: str, db: Session = Depends(get_db), _: User = Depends(get_current_user)):
    rx = db.query(Prescription).filter(Prescription.rx_id == rx_id).first()
    if not rx:
        raise HTTPException(status_code=404, detail="Prescription not found")
    patient = db.query(Patient).filter(Patient.patient_id == rx.patient_id).first() if rx.patient_id else None
